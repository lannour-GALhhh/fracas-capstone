...
dam / risk readings recorded *around* the event — no telemetry is duplicated
onto the event, and no numbers are fabricated. Every block is None-safe:
a source with no readings in the window is reported as unavailable rather than
zeroed, mirroring the risk engine's `available` convention.
"""

from datetime import timedelta

from dam_level.models import DamReading
...
def _peak_dam(lo, hi):
    """The highest dam level in the window (Pasonanca is city-wide)."""
    reading = (
        DamReading.objects.filter(recorded_at__range=(lo, hi))
        .order_by("-water_level")
        .first()
    )
    if reading is None:
        return None
    return {
        "peak_level": reading.water_level,
...
def build_event_telemetry(event, window_hours: int = DEFAULT_WINDOW_HOURS) -> dict:
    """Peak rainfall / dam / risk readings in a window around `event.occurred_at`.
